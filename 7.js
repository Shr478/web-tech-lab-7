function convertJsonToObject() {
  const jsonInput = document.getElementById('jsonInput').value; // ✅ fixed: should be .value not .ariaValueMax
  try {
...
function convertJsonToDate() {
  const dateInput = document.getElementById('dateInput').value; // ✅ fixed: correct element id
  try {
...
function convertJsonToCsv() {
  ...
  try {
    ...
    const headers = Object.keys(jsonArray[0]); // ✅ fixed: used () not []
    const csvRows = [
      headers.join(','),
...
function convertCsvToJson() {
  ...
  document.getElementById('jsonOutput').textContent = JSON.stringify(jsonArray, null, 2); // ✅ fixed: comma not dot
}
...
async function createHash() {
  ...
  const hashBuffer = await crypto.subtle.digest('SHA-256', data); // ✅ fixed: 'data' not 'date'
  const hashArray = Array.from(new Uint8Array(hashBuffer));
  const hashHex = hashArray.map(b => b.toString(16).padStart(2, '0')).join(''); // ✅ fixed: join at the end
